...
class FormularioCliente(ctk.CTkToplevel):
    def __init__(self, parent, callback, datos_editar=None):
        ...
        self.geometry("400x650" if datos_editar else "400x580") 
        ...
        # ============================================================
        if not datos_editar:
            ctk.CTkLabel(self, text="Tipo de Pago:", text_color=C_TEXTO_SECUNDARIO, font=FONT_NORMAL, anchor="w").pack(fill="x", padx=40, pady=(10,0))
    ...
    def _calcular_proximo_ciclo(self, fecha_actual):
        """Aumenta exactamente un mes respetando el día del ciclo."""
    ...
    def _enviar(self):
        ...
        if self.datos_editar:
            ...
            if fecha_inicio != "HOY" and (len(fecha_inicio) != 10 or "/" not in fecha_inicio):
                messagebox.showerror("Error", "Usa el formato DD/MM/AAAA")
                return
            self.callback(nom, tel, self.var_plan.get(), fecha_inicio)
        self.destroy()